...
function render_auditor_view_v5() {
    ...
    <style>
        ...
        .audit-switch[data-state="pending"] {
            background-color: #ffc107;
            color: #fff;
        }

        .audit-switch[data-state="found"] {
            background-color: #4caf50;
            color: #fff;
        }

        .audit-switch[data-state="not-found"] {
    ...
    <div class="inventory-filter-form" style="margin-bottom: 30px; padding: 20px; background-color: #f9f9f9; border-radius: 8px;">
        <form id="location-filter-form" method="GET" action="">
            ...
            <select name="filter_location" id="filter_location">
                ...
                <option value="<?php echo esc_attr($location->term_id); ?>" <?php selected(
    ...
    $all_location_ids = array_merge([$current_location_filter], $child_locations);
    ...
    <p>
        <strong>Audit Session ID:</strong> <?php echo $audit_session_id; ?>
    </p>
    <div id="audit-form">
        ...
        <div class="audit-list">
            ...
            while ($stock_entries_query->have_posts()) {
                ...
                ? DateTime::createFromFormat("Ymd", get_field("purchase_date"))
                : null;
                ...
                for ($i = 0; $i < $quantity; $i++) {
                    ...
                    <div class="audit-item" data-full-id="<?php echo esc_attr(
                        ...
                        <div class="audit-switch" data-state="pending">
                            Pending
                        </div>
                        ...
                        <div class="audit-revision-wrapper">
                            <div class="needs-revision-form">
                                ...
                                <p>
                                    <label>Notes:</label><br><textarea class="audit-notes" rows="2" style="width:100%;"></textarea>
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('verify-location-btn').addEventListener('click',
                function() {
                    saveAllAuditData(this);
                });
                ...
                for (const item of auditItems) {
                    ...
                    if (state === 'found' && condition === 'Needs Revision' && photoFile) {
                        ...
                        const options = {
                            ...
                            useWebWorker: true,
                            // Use a separate thread for performance
                        }
                        try {
...
function handle_ajax_save_single_audit_log() {
    check_ajax_referer("audit_save_nonce",
        "nonce");
    global $wpdb;
